...
| `bifrost.client.enforceAuthOnInference` | Require auth (virtual key, API key, or user token) on `/v1/*` inference routes | `false` |
| `bifrost.client.maxRequestBodySizeMb` | Maximum allowed request body size | `100` |
| `bifrost.client.routingChainMaxDepth` | Maximum depth for routing rule chain evaluation | `10` |
...
bifrost:
  client:
    ...
    enforceAuthOnInference: true   # Every request must carry auth (virtual key, API key, or user token)
...
helm install bifrost bifrost/bifrost \
  ...
  --set bifrost.client.enforceAuthOnInference=true \
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `bifrost.governance.authConfig.isEnabled` | Enable username/password auth for the API and dashboard | `false` |
| `bifrost.governance.authConfig.adminUsername` | Admin username (plain text, prefer secret) | `""` |
| `bifrost.governance.authConfig.adminPassword` | Admin password (plain text, prefer secret) | `""` |
| `bifrost.governance.authConfig.existingSecret` | Kubernetes Secret name for credentials | `""` |
| `bifrost.governance.authConfig.usernameKey` | Key within the secret for username | `"username"` |
| `bifrost.governance.authConfig.passwordKey` | Key within the secret for password | `"password"` |
| `bifrost.governance.authConfig.disableAuthOnInference` | Skip auth check on `/v1/*` inference routes | `false` |

<Warning>
**Deprecated path:** The top-level `bifrost.authConfig` is still accepted for backwards compatibility but `bifrost.governance.authConfig` is the canonical location and takes priority when both are set.
</Warning>
...
| `bifrost.mcp.toolSyncInterval` | Global tool sync interval (Go duration string) | `10m` |
